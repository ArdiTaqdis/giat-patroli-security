...
function generateColumns() {
  ...
  base.push({
    ...
    render: (d) => {
      if (d === "Done")
        return `<span style="color: white; background: #4caf50; padding: 4px 10px; border-radius: 8px;">${d}</span>`;
      if (d === "Expired")
        return `<span style="color: white; background: #f44336; padding: 4px 10px; border-radius: 8px;">${d}</span>`;
      return `<span style="color: white; background: #ff9800; padding: 4px 10px; border-radius: 8px;">${
        d || "Proses"
      }</span>`;
    },
  });
